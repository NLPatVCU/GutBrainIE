...
if [ $# -lt 4 ]; then
	...
	python main.py fit --config=configs/individual/plat_plus_no_cnn_config.yaml
	python main.py predict --config=configs/individual/plat_plus_no_cnn_config.yaml --ckpt_path checkpoints/plat_plus_no_cnn.ckpt
	python postprocess/postprocessing.py predictions.pkl testData.json
