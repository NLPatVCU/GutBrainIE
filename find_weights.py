...
def objective(trial):
    ...
    for i in range(len(all_probs)):
        weights_per_class = [trial.suggest_float(f"weight_model_{i}_class_{j}", 0.0, 1.0) for j in range(len(label_to_int))]
        weights_per_class = np.array(weights_per_class) / np.sum(weights_per_class)
        weights.append(weights_per_class)
...
best_weights = []
for i in range(len(all_probs)):
    best_weights.append([best_params[f"weight_model_{i}_class_{j}"] for j in range(len(label_to_int))])
best_weights = np.array(best_weights)
norm_best_weights = []
for i in range(len(all_probs)):
    norm_best_weights.append([best_weights[i][j] / np.sum(best_weights[i]) for j in range(len(label_to_int))])

best_f1 = -study.best_value

print(norm_best_weights)
print(best_f1)


pickle.dump(norm_best_weights, open("best_weights_ensemble.pkl", "wb"))
